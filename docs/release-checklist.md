...
Tag-triggered draft preparation additionally uses `--require-authored-licenses`. Undeclared CLI/package, canonical or agent-source licenses prevent draft creation. The owner approved MIT for authored CLI, canonical/registry and agent content (canonical ADR-007). Preserve third-party terms and review compatibility/attribution when inputs change; file presence and SPDX collection alone are not legal approval. CI candidate bundles can be inspected while that decision remains open. Published candidate assets consist of complete ZIPs and ZIP checksums so attribution travels with the binary.
