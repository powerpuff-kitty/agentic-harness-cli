...
| `architecture enforce [TARGET]` | Contract preview; `--write` saves it; supports profiles and an explicit date |\n| `quality detect [TARGET]` | Read-only language/tool/config/script discovery; discovered tools remain `executed: false` |\n| `quality analyze [TARGET]` | Read-only Quality Analysis v1 with normalized findings and explicit performed/not-checked/unsupported coverage |
| `quality baseline [TARGET] [--output PATH]` | Snapshot measured findings plus project-contract/tool-config identity; no project command execution |
| `quality diff BASELINE [TARGET] [--output PATH]` | Compare current measured findings to a repository-local baseline and report identity staleness |
...
The architecture indicator is `100 - min(100, deterministic_errors * 100 / supported_source_files)`, a heuristic rather than a calibrated health score. It is unavailable without a supported complete graph.\n\nQuality detection/analyze never executes discovered project commands or installs missing tools. The first quality slice detects common JS/TS/Rust/Python/Go markers and quality configuration, and statically inspects plain-JSON TypeScript `compilerOptions.strict`. JSONC/tsconfig inheritance and decomposed strictness are explicit coverage gaps. Formatter, linter, typecheck, complexity, duplication, dead-code and autofix execution remain not checked. `quality baseline` and `quality diff` operate only on measured evidence and fail closed on malformed baseline/project-contract identity; repository-local artifact writes use confined atomic output. `ah audit` carries quality evidence without manufacturing a `code_quality` score.
