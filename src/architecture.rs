...
fn has_extension(root: &Path, extension: &str) -> bool {
    fn walk(path: &Path, extension: &str, depth: usize) -> bool {
        ...
        for entry in entries.flatten() {
            ...
            if ["node_modules", ".git", "dist", "build", ".nuxt", ".next", "target"].contains(&name) {
                continue;
            }
            if path.is_dir() {
...
fn infer_structure(root: &Path) -> Value {
    ...
    let technical = ["components", "services", "stores", "views", "pages", "api", "composables", "utils"]
        .iter()
        .filter(|name| dirs.contains(**name))
        .map(|name| (*name).to_string())
        .collect::<Vec<_>>();
    let features = ["features", "domains", "modules"]
        .iter()
        .filter(|name| dirs.contains(**name))
        .map(|name| (*name).to_string())
        .collect::<Vec<_>>();
    let layered = ["presentation", "application", "domain", "infrastructure"]
        .iter()
        .filter(|name| dirs.contains(**name))
        .map(|name| (*name).to_string())
        .collect::<Vec<_>>();
    ...
    } else if ports_adapters || (layered.contains(&"domain".to_string()) && layered.contains(&"application".to_string()) && layered.contains(&"infrastructure".to_string())) {
        (
            "hexagonal-or-clean",
            "medium",
...
pub fn detect(root: &Path) -> Value {
    ...
    let has_nuxt = packages.contains("nuxt") || has_any(root, &["nuxt.config.ts", "nuxt.config.js", "nuxt.config.mjs"]).is_some();
    let has_angular = packages.contains("@angular/core") || root.join("angular.json").exists();
    let has_next = packages.contains("next") || has_any(root, &["next.config.js", "next.config.mjs", "next.config.ts"]).is_some();
    let has_vue = packages.contains("vue") || has_extension(root, "vue");
    let has_react = packages.contains("react") || has_extension(root, "jsx") || has_extension(root, "tsx");

    if has_nuxt {
        let mut evidence = Vec::new();
        if packages.contains("nuxt") { evidence.push("package.json:nuxt".to_string()); }
        if let Some(path) = has_any(root, &["nuxt.config.ts", "nuxt.config.js", "nuxt.config.mjs"]) { evidence.push(path); }
        frameworks.push(record("framework", "nuxt", "high", evidence));
    } else if has_angular {
        let mut evidence = Vec::new();
        if packages.contains("@angular/core") { evidence.push("package.json:@angular/core".to_string()); }
        if root.join("angular.json").exists() { evidence.push("angular.json".to_string()); }
        frameworks.push(record("framework", "angular", "high", evidence));
    } else if has_next {
        let mut evidence = Vec::new();
        if packages.contains("next") { evidence.push("package.json:next".to_string()); }
        if let Some(path) = has_any(root, &["next.config.js", "next.config.mjs", "next.config.ts"]) { evidence.push(path); }
        frameworks.push(record("framework", "next", "high", evidence));
    } else if has_vue {
        let mut evidence = Vec::new();
        if packages.contains("vue") { evidence.push("package.json:vue".to_string()); }
        if has_extension(root, "vue") { evidence.push("source:**/*.vue".to_string()); }
        frameworks.push(record("framework", "vue", "high", evidence));
    } else if has_react {
        let mut evidence = Vec::new();
        if packages.contains("react") { evidence.push("package.json:react".to_string()); }
        frameworks.push(record("framework", "react", if packages.contains("react") { "high" } else { "medium" }, evidence));
    }

    if packages.contains("vite") || has_any(root, &["vite.config.ts", "vite.config.js", "vite.config.mjs"]).is_some() {
        let mut evidence = Vec::new();
        if packages.contains("vite") { evidence.push("package.json:vite".to_string()); }
        if let Some(path) = has_any(root, &["vite.config.ts", "vite.config.js", "vite.config.mjs"]) { evidence.push(path); }
    ...
    if packages.contains("nx") || packages.contains("@nx/devkit") || root.join("nx.json").exists() {
        let mut evidence = Vec::new();
        if packages.contains("nx") { evidence.push("package.json:nx".to_string()); }
        if packages.contains("@nx/devkit") { evidence.push("package.json:@nx/devkit".to_string()); }
        if root.join("nx.json").exists() { evidence.push("nx.json".to_string()); }
    ...
    for (package, name, kind) in [
        ...
        if packages.contains(package) {
            ecosystem.push(record(kind, name, "high", vec![format!("package.json:{package}")]));
        }
    }

    if packages.contains("typescript") || root.join("tsconfig.json").exists() {
        let mut evidence = Vec::new();
        if packages.contains("typescript") { evidence.push("package.json:typescript".to_string()); }
        if root.join("tsconfig.json").exists() { evidence.push("tsconfig.json".to_string()); }
        languages.push(record("language", "typescript", "high", evidence));
    } else if root.join("package.json").exists() {
        languages.push(record("language", "javascript", "medium", vec!["package.json".to_string()]));
    }

    let mut candidate_profiles = Vec::new();
    for item in &frameworks {
        if let Some(name) = item.get("name").and_then(Value::as_str) {
            candidate_profiles.push(format!("framework/{name}"));
        }
    }
    for item in &tooling {
        if let Some(name) = item.get("name").and_then(Value::as_str) {
            candidate_profiles.push(format!("tooling/{name}"));
        }
    }
    for item in &ecosystem {
        if let Some(name) = item.get("name").and_then(Value::as_str) {
            candidate_profiles.push(format!("ecosystem/{name}"));
        }
    }
    ...
    if let Some(style) = structure.get("style").and_then(Value::as_str) {
        if style != "unknown" && style != "mixed" {
            candidate_profiles.push(format!("pattern/{style}"));
        }
    }
    ...
    json!({
        ...
        "notes": [
            ...
            "Unknown or mixed structures are not coerced into a framework-independent architecture profile."
        ]
    })
}
...
mod tests {
    ...
    fn detects_vue_vite_pinia_router_and_technical_layers() {
        ...
        assert!(result["tooling"].as_array().unwrap().iter().any(|item| item["name"] == "vite"));
        assert!(result["ecosystem"].as_array().unwrap().iter().any(|item| item["name"] == "pinia"));
    ...
    fn detects_nuxt_as_framework_and_features_as_feature_first() {
        ...
        fs::write(root.join("package.json"), r#"{"dependencies":{"nuxt":"^4.0.0","vue":"^3.0.0"}}"#).unwrap();
        fs::write(root.join("nuxt.config.ts"), "export default defineNuxtConfig({})").unwrap();
    ...
    #[test]
    fn detects_angular_from_official_workspace_markers() {
        let root = fixture("angular");
        fs::write(root.join("package.json"), r#"{"dependencies":{"@angular/core":"^21.0.0","@angular/router":"^21.0.0"}}"#).unwrap();
        ...
        assert!(result["ecosystem"].as_array().unwrap().iter().any(|item| item["name"] == "angular-router"));
        let _ = fs::remove_dir_all(root);
    }
}
