use crate::{design_analysis, design_diff, design_genome, design_prompt};
...
fn write_json_output(value: &Value, output: Option<PathBuf>) {
    let serialized = serde_json::to_string_pretty(value)
        .expect("design output serialization must succeed")
        + "\n";
...
fn read_json(path: &Path) -> Value {
    ...
    serde_json::from_str(&text)
        .unwrap_or_else(|error| die(format!("{} is not valid JSON: {error}", path.display())))
}
                ...
                match args[i].as_str() {
                    "--level" => {
                        ...
                        level = args
                            .get(i)
                            .cloned()
                            .unwrap_or_else(|| die("--level requires a value"));
                    }
                    "--output" => {
                        i += 1;
                        output = Some(PathBuf::from(
                            args.get(i)
                                .cloned()
                                .unwrap_or_else(|| die("--output requires a value")),
                        ));
                    }
        ...
        "preserve" => {
            ...
            while i < args.len() {
                match args[i].as_str() {
                    "--analysis" => {
                        i += 1;
                        analysis = Some(PathBuf::from(
                            args.get(i)
                                .cloned()
                                .unwrap_or_else(|| die("--analysis requires a value")),
                        ));
                    }
                    "--output" => {
                        i += 1;
            ...
            let analysis_path = analysis
                .unwrap_or_else(|| die("preserve requires --analysis DESIGN-ANALYSIS.json"));
        ...
        "prompt" => {
            ...
            while i < args.len() {
                match args[i].as_str() {
                    "--genome" => {
                        i += 1;
                        genome = Some(PathBuf::from(
                            args.get(i)
                                .cloned()
                                .unwrap_or_else(|| die("--genome requires a value")),
                        ));
                    }
                    "--task" => {
                        i += 1;
                        task = Some(PathBuf::from(
                            args.get(i)
                                .cloned()
                                .unwrap_or_else(|| die("--task requires a value")),
                        ));
                    }
                    "--output" => {
                        i += 1;
            ...
            let genome_path =
                genome.unwrap_or_else(|| die("prompt requires --genome DESIGN-GENOME.json"));
