...
fn decorate(root: &Path, mut analysis: Value) -> Value {
    ...
    let (score, coverage) = if source_files > 0 {
        let discoverability_penalty = if contract_present || docs_present { 0 } else { 5 };
        let score = clamp(
            100 - (deterministic_errors * 20).min(80)
                - (warnings * 5).min(20)
                - unresolved.min(10)
                - discoverability_penalty,
        );
        (score, "supported-source-graph")
    } else {
        let base = if contract_present {
            70
        } else if docs_present {
            65
        } else {
            50
        };
        (
            clamp(base - (deterministic_errors * 25).min(50) - (warnings * 5).min(15)),
    ...
    analysis["coverage"] = json!({
        ...
        "score_cap_without_supported_source_graph": 70
    });
    analysis
}
...
mod tests {
    ...
    fn cycle_reduces_architecture_score() {
        ...
        write(&clean, "src/b.ts", "import { a } from './a'; export const b = a;");
        let clean_audit = audit(&clean);

        let cycle = fixture("cycle");
        write(&cycle, "src/a.ts", "import { b } from './b'; export const a = b;");
        write(&cycle, "src/b.ts", "import { a } from './a'; export const b = a;");
    ...
    fn unsupported_graph_cannot_receive_high_score_from_docs_alone() {
        ...
        assert!(result["score"].as_i64().unwrap() <= 70);
        assert_eq!(result["coverage"]["level"], "limited-no-supported-source-graph");
